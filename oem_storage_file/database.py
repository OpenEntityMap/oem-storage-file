...
class DatabaseFileStorage(DatabaseStorage, BaseFileStorage, Plugin):
    ...
    def __init__(self, parent, source, target, version=None, path=None):
        self.parent = parent
        self.source = source
        self.target = target
    ...
    def initialize(self, client):
        ...
        if self.path is None:
            self.path = self.database_path(self.source, self.target, self.version)

    @classmethod
    def open(cls, parent, source, target, version=None, path=None):
        storage = cls(parent, source, target, version, path)
